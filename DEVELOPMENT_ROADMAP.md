...
- [ ] document acknowledgements for under-18s: when a parent acknowledges a document it should propagate to all linked child accounts. Add `acknowledged_by TEXT REFERENCES users(id)` to `document_acknowledgements`; in `POST /api/documents/:id/acknowledge` query `eois` for children via `parent_guardian_email = parent.email` and upsert their records with `acknowledged_by = parent.id`; surface "via [Parent Name]" in the admin acknowledgements view.
